...
const ManagePublications = () => {
    ...
    const handleAddSection = async () => {
        if (newSection.name_en && newSection.name_ru) {
            const docRef = await addDoc(collection(db, "publicationSections"), newSection);
    ...
    const handleUpdateSection = async (id, updatedSection) => {
        await updateDoc(doc(db, "publicationSections", id), updatedSection);
        setSections(sections.map(s => (s.id === id ? { ...s, ...updatedSection } : s)));
    };

    const handleDeleteSection = async (id) => {
        await deleteDoc(doc(db, "publicationSections", id));
        setSections(sections.filter(s => s.id !== id));
    };

    const handleAddPhoto = async () => {
        if (newPhoto.sectionId && newPhoto.original && newPhoto.thumbnail) {
            const docRef = await addDoc(collection(db, `publicationSections/${newPhoto.sectionId}/photos`), {
    ...
    const handleDeletePhoto = async (sectionId, photoId) => {
        await deleteDoc(doc(db, `publicationSections/${sectionId}/photos`, photoId));
        setSections(sections.map(s => {
            if (s.id === sectionId) {
    ...
    return (
        <div className="space-y-8 p-6">
            <h2 className="text-3xl font-bold">{t("admin.publications")}</h2>
